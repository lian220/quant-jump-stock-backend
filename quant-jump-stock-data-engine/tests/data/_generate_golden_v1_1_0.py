"""golden_v1_1_0.csv 생성 스크립트.

scoring_spec.yaml spec_version 1.1.0 에 대한 frozen 동작을 캡처.
...
# PR 3b (v1.1.0): negative AI veto 케이스 — 9 항 (case_id, scenario, ai, tech, sent, has_ai, has_tech, has_sent, signals, veto_reasons)
# 기존 50 케이스는 veto_reasons=() 디폴트로 적용됨 (아래에서 normalize).
VETO_CASES = [
    # F1 - all_max + negative AI veto: 평소 7.40 / S / STRONG → 0 / D / NONE
    (51, "veto_all_max_with_negative_ai", "10.0", "3.5", "10.0", True, True, True, 3, ("ai_negative",)),
    # F2 - 평범 종목 + veto: 평소 5.90 → 0
    (52, "veto_mid_with_negative_ai", "10.0", "3.5", "5.0", True, True, True, 3, ("ai_negative",)),
    # F3 - tech=0 + veto: 평소 4.40 → 0
    (53, "veto_tech_zero_with_negative_ai", "10.0", "0", "5.0", True, True, True, 0, ("ai_negative",)),
    # F4 - 미달 종목 + veto: 평소 1.40 → 0 (이미 D)
    (54, "veto_low_grade_with_negative_ai", "0", "3.5", "0", True, True, False, 3, ("ai_negative",)),
    # F5 - sentiment 만 + veto (모서리)
    (55, "veto_sentiment_only_with_negative_ai", "0", "0", "10.0", False, False, True, 0, ("ai_negative",)),
]
# 기존 50 케이스는 veto_reasons=() 로 정규화 (튜플 길이 통일)
CASES = [
    (c if len(c) == 10 else (*c, ()))
    for c in CASES
] + VETO_CASES


def main():
    out_path = Path(__file__).parent / "golden_v1_1_0.csv"
    policy = ScoringPolicy.load_default()

    rows = []
    for tup in CASES:
        (cid, scenario, ai, tech, sent, has_ai, has_tech, has_sent, signals, veto) = tup
    ...
    fieldnames = [
        ...
        "has_ai", "has_tech", "has_sentiment", "tech_signals", "veto_reasons",
