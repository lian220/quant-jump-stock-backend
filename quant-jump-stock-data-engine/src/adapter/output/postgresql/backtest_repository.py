    ...
    def _insert_result(self, cursor, result: BacktestResult, request_id: Optional[str] = None) -> int:
        ...
        cursor.execute(
            ...
            (
                ...
                float(result.expectancy) if result.expectancy else None,
                float(result.kelly_percentage) if result.kelly_percentage else None,
                float(result.risk_reward_ratio) if result.risk_reward_ratio else None,
                float(result.calmar_ratio) if result.calmar_ratio else None,
                float(result.total_commission) if result.total_commission else None,
                float(result.total_slippage) if result.total_slippage else None,
                float(result.total_tax) if result.total_tax else None,
                float(result.net_profit_after_costs) if result.net_profit_after_costs else None,
                float(result.avg_holding_days) if result.avg_holding_days else None,
                float(result.best_trade) if result.best_trade else None,
                float(result.worst_trade) if result.worst_trade else None,
    ...
    def _insert_trades(self, cursor, result_id: int, trades: list[BacktestTrade]):
        ...
        for trade in trades:
            values.append((
                ...
                float(trade.slippage_amount) if trade.slippage_amount else None,
                float(trade.tax_amount) if trade.tax_amount else None,
                float(trade.execution_price) if trade.execution_price else None,
            ))
