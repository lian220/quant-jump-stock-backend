...
22:00 → 경제 데이터 수집 (EconomicDataUpdate2JobAdapter)
        📊 미국 경제 지표 발표 타이밍 최적화
        - CPI/NFP 발표 (21:30) 직후
        - ISM PMI 발표 (23:00) 직전
...
23:45 → Vertex AI 예측 (VertexAIPredictionJobAdapter)
        🤖 경제 데이터 + 기술적 분석 완료 후 실행
        - Pub/Sub → Data Engine → Vertex AI CustomJob
        - GPU: NVIDIA Tesla T4 (n1-standard-4)
        - 요일별 학습 모드 자동 전환:
          월~토: Fine-tuning (5 epochs, lr=0.00005, 3-5분)
          일요일: Full Training (50 epochs, lr=0.0001, 20-30분)
        - GCS 모델 저장 (gs://.../ml-models/latest/)
        - 예측 결과 MongoDB 저장
        - 조건: gcp.vertex-ai.enabled=true
...
|---|----------|----------|-------------|------|------|
| 1 | economicDataUpdate2Job | 22:00 (매일) | `0 0 22 * * ?` | 경제 데이터 수집 | ✅ 완료 |
| 1.5 | vertexAIPredictionJob | 23:45 (매일) | `0 45 23 * * ?` | Vertex AI 예측 (Fine-tuning/Full Training) | ✅ 완료 |
| 2 | parallelAnalysisJob | 23:05 (매일) | `0 5 23 * * ?` | 기술적 분석 + 감정 분석 병렬 실행 | ✅ 완료 |
| 3 | autoBuyJob | 00:30 (매일) | `0 30 0 * * ?` | 자동 매수 (장 개장 1시간 후) | ✅ 완료 |
| 4 | cleanupOrdersJob | 06:30 (매일) | `0 30 6 * * ?` | 미체결 주문 정리 | ✅ 완료 |
| 5 | portfolioProfitReportJob | 07:00 (매일) | `0 0 7 * * ?` | 포트폴리오 수익률 보고 | ✅ 완료 |
| 6 | autoSellJob | **매 1분** | SimpleSchedule | 자동 매도 체크 (미국 시장 시간만) | ✅ 완료 |

> 분석 기능 상세: [데이터수집](../features/데이터수집.md) | [기술적분석](../features/기술적분석.md) | [뉴스감정분석](../features/뉴스감정분석.md) | [AI분석](../features/인공지능분석.md)
...
| 우선순위 | 방식 | 특징 |
|---------|------|------|
| 1순위 | **Slack Bot API** (`chat.postMessage`) | threadTs 획득 → 스레드 답글로 진행 상황 추적 |
| 2순위 | **Slack Webhook** (fallback) | Bot API 실패 시 자동 전환, threadTs 없음 (단건 알림) |

#### 필수 환경변수

```bash
SLACK_ENABLED=true                    # Slack 알림 활성화
SLACK_BOT_TOKEN=xoxb-...             # Bot API 인증 (스레드 지원)
SLACK_CHANNEL=C0A1XASTLH2            # 알림 채널
SLACK_WEBHOOK_URL=https://hooks...   # Webhook fallback URL
SLACK_WEBHOOK_URL_ERROR=https://...  # 에러 전용 채널 Webhook
```
...
```
[#trading 채널]
├─ 🚀 Vertex AI 예측 작업 시작          ← Core API (Bot API → threadTs 획득)
│   Request ID: 7c4aba01-...
│   Mode: Fine-tuning
│
├─ 🔄 학습 진행 중 (5/5 epoch)          ← predict_optimized.py (스레드 답글)
│
└─ ✅ 예측 작업 완료                      ← Callback (스레드 답글)
    Duration: 4분 32초
    예측 종목: 35개
```
...
| 증상 | 원인 | 해결 |
|------|------|------|
| threadTs가 빈 문자열 | `SLACK_ENABLED=false` 또는 Bot Token 미설정 | 환경변수 확인: `docker exec qjs-core env \| grep SLACK` |
| Bot API 실패 → Webhook fallback | Bot Token 만료/잘못된 채널 | Slack App 설정에서 Token 재발급 |
| 알림 없음 | `SLACK_ENABLED=false` | `.env.prod`에 `SLACK_ENABLED=true` 확인 |
...
### 아키텍처
- [스케줄러 아키텍처](../architecture/스케줄러_아키텍처.md) - Quartz 구조
- [이벤트 기반 아키텍처](../architecture/이벤트_기반_아키텍처.md) - Kafka 이벤트
- [시스템 아키텍처](../architecture/시스템_아키텍처.md) - 전체 구조

### 설정 가이드
- [환경설정 가이드](./환경설정_가이드.md) - 환경 구성
- [배포 운영 가이드](./배포_운영_가이드.md) - 프로덕션 배포
- [Slack 설정 가이드](./Slack_설정_가이드.md) - 알림 연동

### 개발 참고
- [Phase3 스펙](../todo/단계3_스펙.md) - 자동 매매 시스템
- [기능 로드맵](../todo/기능_로드맵.md) - 전체 기능 계획
