...
백테스트 엔진에서 `include_fundamentals=True` 옵션으로 로드 가능:

| DSL 코드 | MongoDB 경로 | 설명 |
|----------|-------------|------|
| `per` | `info.trailingPE` | 주가수익비율 |
| `pbr` | `info.priceToBook` | 주가순자산비율 |
| `dividend_yield` | `info.dividendYield` | 배당수익률 |
| `roe` | `info.returnOnEquity` | 자기자본이익률 |
| `earnings_growth` | `info.earningsGrowth` | 이익성장률 |
| `debt_to_equity` | `info.debtToEquity` | 부채비율 |
| `forward_pe` | `info.forwardPE` | 선행 PER |

**평가**:
- ⚠️ close_price만 수집 중 (OHLV 없음)
- ✅ info 필드에 펀더멘탈 데이터 존재 → 백테스트 DSL에서 활용 가능

**권장사항**: OHLCV 전체 데이터 수집 기능 추가 필요
...
**추천 종류**:
- `STRONG BUY`: 2% 이상 상승 예측 & MAPE ≤ 30%
- `BUY`: 0% ~ 2% 상승 예측 & MAPE ≤ 30%
- `HOLD (Low Confidence)`: MAPE > 30% (예측 신뢰도 낮음)
- `SELL`: 하락 예측 & MAPE ≤ 30%

> MAPE 30% 초과 종목은 `prediction_confidence: "low"`로 마킹됨.
...
### 분석 결과
- [FRED 데이터 분석](./프레드_데이터_분석_2026-02-01.md) - FRED 데이터 발표 주기

### 기능 명세
- [분석 아키텍처](../features/ANALYSIS_아키텍처.md) - 분석 시스템 구조
- [predict.py 비교](../features/PREDICT_PY_COMPARISON.md) - 버전 비교
- [모델 비교](../features/MODEL_COMPARISON.md) - ML 모델 비교
...
### 데이터베이스
- [데이터베이스 스키마](../database/스키마.md) - 전체 스키마
- [데이터베이스 관계](../database/관계.md) - ERD 및 관계
