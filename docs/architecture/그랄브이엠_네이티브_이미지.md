...
> **적용일**: 2026-02-15  
> **최종 업데이트**: 2026-03-01  
> **현재 상태**: 프로덕션 운영 중 (GCE VM e2-micro). Cloud Run 전환 시 동일 Native Image 사용 예정 (Terraform 반영 완료).

> **문서 규칙**: GraalVM, JVM, JIT, AOT, SubstrateVM, Native Image 등 기술 용어는 문서 전반에서 영어로 표기합니다.
...
빌드 시 (개발자 PC 또는 CI)
  │
  ├─ native-image 컴파일러 실행 (12GB 메모리 사용, 15–25분 소요, 의존성 많으면 30분+)
  │    │
...
장점:
- **시작 즉시 빠름**: 기계어 로드 → 바로 실행 = ~2.5초
- **메모리 적음**: JVM 런타임 오버헤드 없음 = ~170MB
- **예측 가능한 성능**: 시작부터 끝까지 일정한 속도

단점:
- **빌드 느림**: 전체 코드 정적 분석 = 15–25분(의존성에 따라 30분+)
...
AOT (Native)  15–25분     2초         170MB     ★★★☆☆     제한적
```
...
| 환경 | JVM 빌드 | Native 빌드 (풀 최적화) | Native (quickBuild `-PquickNative`) |
|------|---------|-------------------------|-------------------------------------|
| 로컬 (예: M3 Max) | ~30초 | **~15–25분** | **~2–5분** |
| CI (GitHub Actions) | ~3분 | **~15–25분** (캐시 시 단축) | 권장하지 않음 (프로덕션은 풀 빌드) |

- native-image 컴파일러가 전체 코드를 정적 분석 + 최적화하므로 시간이 오래 걸림
- CI에서 12GB 힙 + 10GB Swap 필요 (`-J-Xmx12g`)
- **대응**: 로컬 개발은 JVM 모드(`./gradlew bootRun`) 또는 **`-PquickNative`**; 프로덕션 Native는 CI에서만 풀 빌드

#### 빌드가 예전보다 느려진 경우

의존성·버전이 늘어나면 **도달 가능한 코드(reachable types/methods)** 가 크게 증가해 빌드 시간이 길어집니다.

| 원인 | 설명 |
|------|------|
| ~~GCP Pub/Sub (`spring-cloud-gcp-starter-pubsub`)~~ | ✅ **제거 완료** (2026-03-01) — `google-cloud-pubsub:1.149.0` 직접 사용으로 전환. gRPC/Netty 자동 초기화 오버헤드 제거 |
| **Spring Boot 3.5.x** | AOT 처리 강화로 초기화·리플렉션 힌트가 늘어남 |
| **SpringDoc OpenAPI** | 컨트롤러·모델 스캔으로 리플렉션 등록이 많아짐 |
| **MongoDB + JPA + Flyway** | 각각 native-image용 설정·리플렉션 추가 |

현재 프로젝트 기준으로 **reachable types ~6만 개, reachable methods ~38만 개** 수준이면 풀 빌드 **20~30분**은 흔한 편입니다.  
예전에 5~12분이었던 것은 의존성이 적었거나, 로컬에서 `-PquickNative`를 썼을 가능성이 있습니다.

- **로컬**: `./start.sh --quick-native --build --backend` 또는 `./gradlew nativeCompile -PquickNative` 사용 권장
- **CI**: Gradle 캐시·native 빌드 캐시 사용 시 두 번째 빌드부터는 단축 가능

#### 빌드 속도 개선 (로컬 한정)

| 방법 | 적용 | 효과 | 비고 |
|------|------|------|------|
| **Quick Build** | `-PquickNative` | 분석/컴파일 단계 일부 스킵 → **수 분 단축** | 프로덕션 이미지보다 최적화 낮음, 로컬 검증용 |
| **최적화 완화 (-O0)** | `-PquickNative` 시 자동 | Compiling 단계 단축 | 위와 함께 사용 |
| **메모리 확대** | CI `-PnativeImageArgs="-J-Xmx12g"` | 빌드 중 GC 감소 → 대기 시간 감소 | 이미 적용 |
| **--verbose 제거** | buildArgs에서 제거 | 로그 I/O 소량 감소 | 체감 미미 |

**로컬에서 빠르게 Native 실행해 보기:**
```bash
./gradlew nativeCompile -x test -PquickNative
./build/native/nativeCompile/quant-jump-stock-core
```
`start.sh`에서 quickNative를 쓰려면 `./start.sh local --dev --native --build-backend --backend` 실행 전에 Core 디렉터리에서 `-PquickNative`를 전달하도록 스크립트를 확장할 수 있다.

**한계**: 전체 프로그램 AOT 분석 자체가 무거우므로, 풀 최적화 빌드는 **15~25분(환경·의존성에 따라 30분 넘을 수 있음)** 수준에서 크게 줄이기 어렵다. 반복 빌드 시 Gradle/네이티브 캐시로 일부 단계만 재실행되면 단축된다.
...
| ✅ 호환 | Kotlin stdlib, coroutines, google-cloud-pubsub (직접 사용) |
...
./gradlew bootRun                  GitHub Actions
  ...
  → 디버거 연결 가능                   → Docker push → VM 배포 (현재)
  → JMX/VisualVM 사용 가능            → 시작 ~2.5초, RSS ~170MB
                                     → Cloud Run 배포 (전환 시, 동일 이미지)
```

**Cloud Run 전환**: Terraform에 Core API Cloud Run 서비스(`qjs-core`)가 정의되어 있으며, 전환 시 동일 Native Image를 Cloud Run에 배포한다. `PORT` 환경변수, `application-cloudrun.yml`, startup/liveness probe 등은 [클라우드런_전환_Phase1.md](./클라우드런_전환_Phase1.md) 참고.
...
VM 배포용 포트 10010, Cloud Run 전환 시에는 `PORT`(기본 8080) 사용. Cloud Run에서는 HEALTHCHECK를 서버가 관리하므로 프로덕션 Dockerfile에서 제거할 수 있음 (Phase1 문서 참고).
...
- [클라우드런 전환 Phase1](./클라우드런_전환_Phase1.md) - Core API Cloud Run 전환 및 Terraform (동일 Native Image 사용)
