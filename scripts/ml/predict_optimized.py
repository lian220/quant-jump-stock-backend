...
pg_host = os.getenv("DB_HOST") or os.getenv("POSTGRES_HOST") or "localhost"
pg_port = os.getenv("DB_PORT") or os.getenv("POSTGRES_PORT") or "5432"
pg_database = os.getenv("DB_NAME") or os.getenv("POSTGRES_DB") or "quantiq"
pg_user = os.getenv("DB_USER") or os.getenv("POSTGRES_USER") or "quantiq_user"
pg_password = os.getenv("DB_PASSWORD") or os.getenv("POSTGRES_PASSWORD") or "quantiq_password"
...
# PostgreSQL 연결 테스트 및 검증
pg_connection_available = False

print("\n=== PostgreSQL 연결 시도 ===")
print(f"DB_HOST 설정 여부: {'설정됨 (' + pg_host + ')' if os.getenv('DB_HOST') else '기본값 사용 (localhost)'}")
print(f"DB_PORT: {pg_port}")
print(f"DB_NAME: {pg_database}")
print(f"DB_USER 설정 여부: {'설정됨' if os.getenv('DB_USER') else '기본값 사용'}")
print(f"DB_PASSWORD 설정 여부: {'설정됨' if os.getenv('DB_PASSWORD') else '기본값 사용'}")
...
if not os.getenv("DB_HOST"):
    ...
    send_slack_notification(
        status="FAILED",
        message="PostgreSQL 환경변수(DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD)가 설정되지 않았습니다.",
...
try:
    ...
    print("PostgreSQL 연결 상태: ❌ 미연결")
    # 연결 실패 시 Slack 알림 전송 후 종료
    send_slack_notification(
        status="FAILED",
        message=f"PostgreSQL 연결 실패: {pg_host}:{pg_port}/{pg_database}",
        error_detail=str(e)
    )
    print("\n❌ PostgreSQL 연결 필수 - 스크립트 종료")
    exit(1)

print(f"PostgreSQL 연결 상태: ✅ 연결됨")

# ============================================
# Slack 알림 설정 (작업 완료 알림용)
# ============================================
import requests
import time

slack_bot_token = os.getenv("SLACK_BOT_TOKEN")
slack_channel = os.getenv("SLACK_CHANNEL") or "#trading-alerts"
slack_thread_ts = os.getenv("SLACK_THREAD_TS")
request_id = os.getenv("REQUEST_ID") or f"vertex-ai-{int(time.time())}"
job_start_time = time.time()

def send_slack_notification(status: str, message: str = None, error_detail: str = None, predicted_count: int = None):
    """Slack으로 작업 완료 알림 전송"""
    if not slack_bot_token:
        print("⚠️ SLACK_BOT_TOKEN이 설정되지 않아 Slack 알림을 전송하지 않습니다.")
        return False

    duration_seconds = int(time.time() - job_start_time)
    duration_str = f"{duration_seconds // 60}분 {duration_seconds % 60}초"

    # 상태에 따른 색상 및 이모지
    if status == "SUCCESS":
        color = "36a64f"  # 녹색
        title = "✅ Vertex AI 예측 작업 완료"
        status_text = "SUCCESS"
    else:
        color = "dc3545"  # 빨강
        title = "❌ Vertex AI 예측 작업 실패"
        status_text = "FAILED"
    ...
    if predicted_count is not None:
        fields.append({"title": "Predicted Count", "value": f"{predicted_count}개 종목", "short": True})

    if message:
        fields.append({"title": "Message", "value": message, "short": False})

    if error_detail:
        fields.append({"title": "Error", "value": error_detail, "short": False})
    ...
    try:
        ...
        print(f"Thread TS: {slack_thread_ts}")
        ...
        if result.get("ok"):
            ...
            print(f"⚠️ Slack 응답 오류: {result.get('error')}")
            return False
    except Exception as e:
...
def global_exception_handler(exc_type, exc_value, exc_tb):
    ...
    send_slack_notification(
        status="FAILED",
        message=f"예외 발생: {exc_type.__name__}",
...
def get_all_data(collection_name):
    ...
    try:
        ...
        print(f"PostgreSQL에서 활성화된 종목 {len(active_stock_names)}개를 찾았습니다.")
        print(f"활성화된 티커 {len(active_tickers)}개를 찾았습니다.")

        if len(active_stock_names) == 0:
            print("⚠️ 경고: 활성화된 종목이 없습니다.")
            print("stocks 테이블에 is_active=true인 종목이 있는지 확인해주세요.")
        else:
...
def get_target_columns_from_db():
    ...
    try:
        ...
        # PostgreSQL stocks 테이블에서 활성화된 주식 조회
        ...
        print(f"PostgreSQL에서 {len(target_columns)}개의 활성화된 주식을 가져왔습니다.")
        print(f"주식 목록: {target_columns[:10]}... (총 {len(target_columns)}개)")

        return target_columns
    except Exception as e:
        print(f"⚠️ 경고: PostgreSQL에서 주식 목록 조회 중 오류 발생: {str(e)}")
        print("⚠️ 경고: 하드코딩된 기본 주식 목록을 사용합니다. DB 연결을 확인하세요.")
        import traceback
        traceback.print_exc()
        return _get_default_target_columns()
...
def get_economic_features_from_postgres():
    """
    PostgreSQL의 fred_indicators와 yfinance_indicators 테이블에서
    ...
    try:
        ...
        print(f"⚠️ 경고: PostgreSQL 지표 조회 중 오류 발생: {str(e)}")
        import traceback
        traceback.print_exc()
        return []

# PostgreSQL에서 지표 목록 조회
economic_features = get_economic_features_from_postgres()
...
# 11) Slack 완료 알림 전송
print("\n" + "=" * 60)
print("[최종] 작업 완료 - Slack 알림 전송")
print("=" * 60)
try:
    predicted_count = len(final_results) if final_results is not None else 0
    send_slack_notification(
        status="SUCCESS",
        message=f"주가 예측 및 분석 완료. {predicted_count}개 종목 처리됨.",
        predicted_count=predicted_count
    )
    print("✅ 전체 작업 완료!")
except Exception as slack_error:
    print(f"⚠️ Slack 알림 전송 중 오류 (작업은 완료됨): {slack_error}")
